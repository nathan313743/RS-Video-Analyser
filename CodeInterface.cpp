...
{	
	...
	QString str1 = "SELECT DISTINCT f.pk_file_ID, f.file_name"
		...
		" LEFT JOIN Audio a ON a.fk_file_ID = f.pk_file_ID"
		" LEFT JOIN Transport_Stream ts ON ts.fk_file_ID = f.pk_file_ID"
		" LEFT JOIN TS_Data tsd ON tsd.fk_file_ID = f.pk_file_ID"
		" LEFT JOIN Signal_Root sr ON sr.fk_file_ID = f.pk_file_ID"
    " LEFT JOIN Test_Case tc ON tc.fk_file_ID = f.pk_file_ID"
    " LEFT JOIN Interface i ON i.fk_file_ID = f.pk_file_ID"+ clause;
...
QList<QStringList> CodeInterface::get_Signal_Root_vals(int fileID)
{
	return db_access.get_Signal_Root_vals(fileID);
}

QList<QStringList> CodeInterface::get_Test_Case_vals(int fileID)
{
	return db_access.get_Test_Case_vals(fileID);
}

QList<QStringList> CodeInterface::get_Transport_Stream_vals(int fileID)
{
	return db_access.get_Transport_Stream_vals(fileID);
}

QList<QStringList> CodeInterface::get_TS_Data_vals(int fileID)
{
	return db_access.get_TS_Data_vals(fileID);
}
