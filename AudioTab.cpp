...
void AudioTab::createDisplay()
{
	...
	grid->addWidget(codec_group(), 0, 0);
	grid->addWidget(other_group(), 1 ,0);
...
QGroupBox *AudioTab::codec_group()
{
	QGroupBox *groupBox = new QGroupBox(tr("Audio Codec"));
	QFormLayout *layout = new QFormLayout;

	cmb_audio_format = new QComboBox();
	cmb_audio_bitrate = new QComboBox();
...
QGroupBox *AudioTab::other_group()
{
	QGroupBox *groupBox = new QGroupBox();
	QFormLayout *layout = new QFormLayout;

	cmb_channel = new QComboBox();
	cmb_content = new QComboBox();
	cmb_encryption = new QComboBox();
	cmb_language = new QComboBox();
	cmb_sampling = new QComboBox();

	layout->addRow(new QLabel("Channel:"), cmb_channel);
	layout->addRow(new QLabel("Content:"), cmb_content);
	layout->addRow(new QLabel("Encryption:"), cmb_encryption);
	layout->addRow(new QLabel("Language:"), cmb_language);
	layout->addRow(new QLabel("Sampling:"), cmb_sampling);
	groupBox->setLayout(layout);
