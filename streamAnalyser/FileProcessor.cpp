  ...
  QList<FileDetails> processed_list;
	...
	for(int i = 0; i < filePaths.size(); i++)
	{
      ...
      qDebug() << "Cannot open: " << filePaths.at(i);
		...
		transDetails.set_num_aud_service(numAudStreams);
		transDetails.set_num_vid_service(numVidStreams);
		transportProcess(MI, transDetails);
		fd.set_trans_list(transDetails);
...
void FileProcessor::transportProcess(MediaInfo &MI, TransportDetails &trans_details)
{
	String result;
	bool telexText = false;
	bool subtitles = false;

	if(trans_details.get_num_vid_services() == 0)
	{
		trans_details.set_audio_only(true);
	}
	else
	{
		trans_details.set_audio_only(false);
	}

	result = MI.Get(Stream_General, 0, _T("TextCount"), Info_Text, Info_Name).c_str();
	QString num = convertToQString(result);
	int numTextStreams = num.toInt();

	for(int i = 0; i < numTextStreams; i++)
	{
		result = MI.Get(Stream_Text, i, _T("Format"), Info_Text, Info_Name).c_str();
		QString text = convertToQString(result);
		text = text.toLower();
	...
	TransData transData;
	transData.setTeletext(telexText);
	transData.setSubtitle(subtitles);

	trans_details.set_trans_data(transData);
}
